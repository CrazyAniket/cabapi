...
exports.updateByID = (request, result) => {
  ...
  cityObj.update(request.body, {
    ...
    if (num[0] === 1) {
      ...
      result.send({
        message: `Cannot update city object with id=${paramID}!`
      });
    }
  }).catch(err => {
    result.status(500).send({
      message: err.message || `Error while updating city object with id=${paramID}!`
    });
  });
};
...
exports.deleteByID = (request, result) => {
  ...
  cityObj.destroy({
    ...
    if (num === 1) {
      ...
      result.send({
        message: `Cannot delete city object with id=${id}!`
      });
    }
  }).catch(err => {
    result.status(500).send({
      message: err.message || `Cannot delete city object with id=${id}!`
    });
  });
};